...
public sealed class WhatsAppInboxEvent : Entity, ITenantOwnedEntity
{
    private WhatsAppInboxEvent()
    {
    ...
    private WhatsAppInboxEvent(Guid tenantId, Guid? connectionId, string eventId, string payloadJson, DateTime utcNow)
        : this()
    {
    ...
    public static WhatsAppInboxEvent Create(Guid tenantId, Guid? connectionId, string eventId, string payloadJson, DateTime utcNow)
        => new(tenantId, connectionId, eventId, payloadJson, utcNow);
    ...
    public void MarkFailed(string error, DateTime nextAttemptAtUtc, int maxAttempts)
    {
        ...
        Status = AttemptCount >= maxAttempts ? WhatsAppQueueStatus.DeadLetter : WhatsAppQueueStatus.Failed;
        NextAttemptAtUtc = Status == WhatsAppQueueStatus.DeadLetter ? null : nextAttemptAtUtc;
    }

    public void SanitizePayload(DateTime utcNow)
    {
        EnsureUtc(utcNow);
        PayloadJson = "{}";
        ErrorMessage = null;
